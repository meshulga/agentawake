...
func TestWriteTokenRejectsTmpSessionIDSuffix(t *testing.T) {
	s := New(t.TempDir())

	if err := s.WriteToken(newTok("foo.tmp")); err == nil {
		t.Fatalf("WriteToken .tmp session ID: got nil error, want error")
	}
}
